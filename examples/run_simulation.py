...
def add_targets_cluster(
    ...
    @param n: number of targets to add
    @param cluster_domain: domain where the targets will be placed
    @param cluster_spacing: minimum distance between targets
    @param targets_material: material of the targets
    @param targets_radius: radius of the targets
    """
    clusters_points = []
    while True:
...
if __name__ == "__main__":
    ...
    dataset_folder = Path("output")

    for n in tqdm(range(1, 7)):
        dataset_subfolder = dataset_folder.joinpath(f"n{n}")
        for i in range(1, 100):
            run_folder = dataset_subfolder.joinpath(f"run{i}")
            ...
            model.register_materials(
                ...
                Material(id="potato", permittivity=50.0, conductivity=0.01, permeability=1),
            )
            ...
            geometry_plot = model.plot_geometry(return_image=True)
            geometry_plot.save(run_folder.joinpath(f"geometry.png"))

            data_plot = model.plot_data(return_image=True)
            data_plot.save(run_folder.joinpath(f"data.png"))

            with open(run_folder.joinpath("data.txt"), "w") as f:
                for obj in model.geometry:
                    if isinstance(obj, DomainSphere):
                        f.write(f"{obj.cx} {obj.cy} {obj.cz} {obj.volume()}\n")
